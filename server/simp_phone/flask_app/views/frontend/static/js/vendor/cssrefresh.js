 ...
 *	www.frebsite.nl
 *
 *	Dual licensed under the MIT and GPL licenses. Ha!
...
(function() {
  ...
  const refresh_rate = (function () {
    const script_tags = document.getElementsByTagName("script")
    const this_script_tag = script_tags[script_tags.length - 1]
    const refresh_rate_str = this_script_tag.dataset.interval
    const refresh_rate = parseInt(refresh_rate_str)
    if (isNaN(refresh_rate)) {
      throw new Error("'" + refresh_rate_str + "'" +
                      " is not an integer (of ms)")
    }
    return refresh_rate
  }())
	...
	var phpjs = {
      ...
      const last_modified_header = (
        headers && (headers[ 'Last-Modified' ] ||
                    headers[ 'last-modified' ])
      )
			return ( last_modified_header &&
               Date.parse( last_modified_header ) / refresh_rate ) ||
        false;
	    },
